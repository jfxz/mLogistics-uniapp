...
<script>
	...
	import {getVersion} from '../../common/api/version/getVersion.js'
...
      // 获取本地应用资源版本号  
		...
		methods: {
			...
			judgeToken () {
				...
				if (uni.getStorageSync('access_token')) {
					getUserInfo().then(res => {
						if (res.status) {
							this.$store.dispatch('GetAbnType')
							this.$store.dispatch('GetWarehouse')
							this.$store.dispatch('GetCustomerValue')
							this.$store.dispatch('GetSpaceialCodeValue')
							this.$store.dispatch('GetCargoname')
			...
			check (flag) {
				...
				// 	if (res.status) {
				// 		// #ifdef APP-PLUS
				// 		console.log(this.currenVersion, res.message)
				// 		if (this.currenVersion !== res.message) {
