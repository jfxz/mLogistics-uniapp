		...
		<view class="tui-header" :class="{ 'tui-show': isShow }">
			...
			<view class="tui-picker-body">
				<picker-view :value="value" @change="change" class="tui-picker-view">
					...
					<picker-view-column v-if="!reset && (type==1)">
	...
	export default {
		...
		methods: {
			...
			setHours() {
				...
				setTimeout(()=> {
					this.$set(this.value, 3, this.getIndex(this.hours, this.hour));
				}, 8);
			...
			setMinutes() {
				...
				setTimeout(()=> {
					this.$set(this.value, 4, this.getIndex(this.minutes, this.minute));
				}, 8);
			},
			setSeconds() {
				this.seconds = this.generateArray(0, 59);
				setTimeout(()=> {
					this.$set(this.value, 5, this.getIndex(this.seconds, this.second));
				}, 8);
			},
			show() {
				setTimeout(() => {
